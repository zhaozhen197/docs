...
It is very likely that `plone` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run --name=zeo winamd64/plone zeoserver
```

Start 2 Plone clients

```console
$ docker run --link=zeo -e ZEO_ADDRESS=zeo:8100 -p 8081:8080 winamd64/plone
$ docker run --link=zeo -e ZEO_ADDRESS=zeo:8100 -p 8082:8080 winamd64/plone
```
