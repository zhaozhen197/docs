...
It is very likely that `percona` does not support the currently selected architecture (`arm64v8`).
...
```console
$ docker run -it --link some-percona:mysql --rm arm64v8/percona sh -c 'exec mysql -h"$MYSQL_PORT_3306_TCP_ADDR" -P"$MYSQL_PORT_3306_TCP_PORT" -uroot -p"$MYSQL_ENV_MYSQL_ROOT_PASSWORD"'
```

... where `some-percona` is the name of your original `arm64v8/percona` container.
