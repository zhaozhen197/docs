...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm32v6/ruby:2.1 bundle install
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v6/ruby:2.1 ruby your-daemon-or-script.rb
```
