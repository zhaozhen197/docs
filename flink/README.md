...
It is very likely that `flink` does not support the currently selected architecture (`arm32v7`).
...
```console
$ docker run --name flink_local -p 8081:8081 -t arm32v7/flink local
```
...
```console
$ docker run --rm -t arm32v7/flink flink run -m <jobmanager:port> -c <your_class> <your_jar> <your_params>
```
