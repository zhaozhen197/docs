...
It is very likely that `mysql` does not support the currently selected architecture (`ppc64le`).
...
```console
$ docker run -it --link some-mysql:mysql --rm ppc64le/mysql sh -c 'exec mysql -h"$MYSQL_PORT_3306_TCP_ADDR" -P"$MYSQL_PORT_3306_TCP_PORT" -uroot -p"$MYSQL_ENV_MYSQL_ROOT_PASSWORD"'
```

... where `some-mysql` is the name of your original `ppc64le/mysql` container.
