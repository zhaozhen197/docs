...
It is very likely that `mysql` does not support the currently selected architecture (`arm32v6`).
...
```console
$ docker run -it --link some-mysql:mysql --rm arm32v6/mysql sh -c 'exec mysql -h"$MYSQL_PORT_3306_TCP_ADDR" -P"$MYSQL_PORT_3306_TCP_PORT" -uroot -p"$MYSQL_ENV_MYSQL_ROOT_PASSWORD"'
```

... where `some-mysql` is the name of your original `arm32v6/mysql` container.
