...
It is very likely that `oraclelinux` does not support the currently selected architecture (`arm64v8`).
...
The Oracle Linux images are intended for use in the **FROM** field of an application's `Dockerfile`. For example, to use Oracle Linux 6 as the base of an image, specify `FROM arm64v8/oraclelinux:6`.
