...
It is very likely that `photon` does not support the currently selected architecture (`arm64v8`).
...
Photon OS images are intended for use in the **FROM** field of an application's `Dockerfile`. For example, to use VMware Photon 1.0RC as the base of an image, specify `FROM arm64v8/photon:1.0RC`.
