...
It is very likely that `photon` does not support the currently selected architecture (`arm32v5`).
...
Photon OS images are intended for use in the **FROM** field of an application's `Dockerfile`. For example, to use VMware Photon 1.0RC as the base of an image, specify `FROM arm32v5/photon:1.0RC`.
