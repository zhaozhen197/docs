...
It is very likely that `maven` does not support the currently selected architecture (`windows-amd64`).
...
```dockerfile
FROM winamd64/maven:3.2-jdk-7-onbuild
...
```console
$ docker run -it --rm --name my-maven-project -v "$PWD":/usr/src/mymaven -w /usr/src/mymaven winamd64/maven:3.2-jdk-7 mvn clean install
```
