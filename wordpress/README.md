...
It is very likely that `wordpress` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run --name some-wordpress --link some-mysql:mysql -d winamd64/wordpress
```
...
```console
$ docker run --name some-wordpress --link some-mysql:mysql -p 8080:80 -d winamd64/wordpress
```
