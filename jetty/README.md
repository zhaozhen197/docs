...
It is very likely that `jetty` does not support the currently selected architecture (`ppc64le`).
...
```console
$ docker run -d ppc64le/jetty --modules=jmx jetty.threadPool.maxThreads=500
```
