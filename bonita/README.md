...
It is very likely that `bonita` does not support the currently selected architecture (`arm64v8`).
...
2.	Start your `bonita` container like this:
	...
	docker run --name some-bonita -v /my/own/datadir:/opt/bonita -d arm64v8/bonita:tag
...
-	Launch the new container pointing towards the copy of DB and filesystem
	...
	```console
	$ docker run --name=bonita_7.5.4_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -d -p 8081:8080 arm64v8/bonita:7.5.4
	```
