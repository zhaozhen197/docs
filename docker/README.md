...
It is very likely that `docker` does not support the currently selected architecture (`i386`).
...
```console
$ docker run --privileged --name some-docker -d i386/docker:stable-dind
```
...
```console
$ docker run --rm --link some-docker:docker i386/docker:edge version
Client:
 Version:      17.05.0-ce
...
```console
$ docker run -it --rm --link some-docker:docker i386/docker:edge sh
/ # docker version
Client:
 Version:      17.05.0-ce
 API version:  1.27 (downgraded from 1.29)
...
```console
$ docker run --rm --link some-docker:docker i386/docker info
Containers: 0
 Running: 0
 Paused: 0
 Stopped: 0
...
```console
$ docker run --rm -v /var/run/docker.sock:/var/run/docker.sock i386/docker version
Client:
 Version:      17.05.0-ce
...
```console
$ docker run --privileged --name some-overlay-docker -d i386/docker:dind --storage-driver=overlay
```
