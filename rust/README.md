...
It is very likely that `rust` does not support the currently selected architecture (`arm32v5`).
...
```console
$ docker run --rm --user "$(id -u)":"$(id -g)" -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v5/rust:1.19.0 cargo build --release
```
