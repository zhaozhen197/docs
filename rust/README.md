...
It is very likely that `rust` does not support the currently selected architecture (`ppc64le`).
...
```console
$ docker run --rm --user "$(id -u)":"$(id -g)" -v "$PWD":/usr/src/myapp -w /usr/src/myapp ppc64le/rust:1.19.0 cargo build --release
```
