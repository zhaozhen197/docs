...
It is very likely that `composer` does not support the currently selected architecture (`arm32v7`).
...
-	Create your own image, and copy Composer from the official image into it:
	...
	```dockerfile
	COPY --from=arm32v7/composer:1.5 /usr/bin/composer /usr/bin/composer
	```
