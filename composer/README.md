...
It is very likely that `composer` does not support the currently selected architecture (`ppc64le`).
...
-	Create your own image, and copy Composer from the official image into it:
	...
	```dockerfile
	COPY --from=ppc64le/composer:1.5 /usr/bin/composer /usr/bin/composer
	```
