...
It is very likely that `jruby` does not support the currently selected architecture (`arm32v7`).
...
```dockerfile
FROM arm32v7/jruby:1.7-onbuild
...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm32v7/jruby:1.7 bundle install --system
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v7/jruby:1.7 jruby your-daemon-or-script.rb
```
