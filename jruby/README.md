...
It is very likely that `jruby` does not support the currently selected architecture (`windows-amd64`).
...
```dockerfile
FROM winamd64/jruby:1.7-onbuild
...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app winamd64/jruby:1.7 bundle install --system
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/jruby:1.7 jruby your-daemon-or-script.rb
```
