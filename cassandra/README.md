...
[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/cassandra/badge/icon) (`i386/cassandra` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/cassandra/)
...
```console
$ docker run --name some-cassandra -d i386/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d -e CASSANDRA_SEEDS="$(docker inspect --format='{{ .NetworkSettings.IPAddress }}' some-cassandra)" i386/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --link some-cassandra:cassandra i386/cassandra:tag
```
...
```console
$ docker run -it --link some-cassandra:cassandra --rm i386/cassandra sh -c 'exec cqlsh "$CASSANDRA_PORT_9042_TCP_ADDR"'
```

... or (simplified to take advantage of the `/etc/hosts` entry Docker adds for linked containers):

```console
$ docker run -it --link some-cassandra:cassandra --rm i386/cassandra cqlsh cassandra
```
