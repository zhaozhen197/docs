...
-	[`2.1.19`, `2.1` (*2.1/Dockerfile*)](https://github.com/docker-library/cassandra/blob/88c25000ef8da0e0f7eb3a6b8202b2f7678c8dab/2.1/Dockerfile)
-	[`2.2.11`, `2.2`, `2` (*2.2/Dockerfile*)](https://github.com/docker-library/cassandra/blob/88c25000ef8da0e0f7eb3a6b8202b2f7678c8dab/2.2/Dockerfile)
-	[`3.0.15`, `3.0` (*3.0/Dockerfile*)](https://github.com/docker-library/cassandra/blob/88c25000ef8da0e0f7eb3a6b8202b2f7678c8dab/3.0/Dockerfile)
-	[`3.11.1`, `3.11`, `3`, `latest` (*3.11/Dockerfile*)](https://github.com/docker-library/cassandra/blob/88c25000ef8da0e0f7eb3a6b8202b2f7678c8dab/3.11/Dockerfile)

[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/cassandra/badge/icon) (`amd64/cassandra` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/cassandra/)
...
```console
$ docker run --name some-cassandra -d amd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d -e CASSANDRA_SEEDS="$(docker inspect --format='{{ .NetworkSettings.IPAddress }}' some-cassandra)" amd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --link some-cassandra:cassandra amd64/cassandra:tag
```
...
```console
$ docker run -it --link some-cassandra:cassandra --rm amd64/cassandra sh -c 'exec cqlsh "$CASSANDRA_PORT_9042_TCP_ADDR"'
```

... or (simplified to take advantage of the `/etc/hosts` entry Docker adds for linked containers):

```console
$ docker run -it --link some-cassandra:cassandra --rm amd64/cassandra cqlsh cassandra
```
