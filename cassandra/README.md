...
It is very likely that `cassandra` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run --name some-cassandra -d winamd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d -e CASSANDRA_SEEDS="$(docker inspect --format='{{ .NetworkSettings.IPAddress }}' some-cassandra)" winamd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --link some-cassandra:cassandra winamd64/cassandra:tag
```
...
```console
$ docker run -it --link some-cassandra:cassandra --rm winamd64/cassandra sh -c 'exec cqlsh "$CASSANDRA_PORT_9042_TCP_ADDR"'
```

... or (simplified to take advantage of the `/etc/hosts` entry Docker adds for linked containers):

```console
$ docker run -it --link some-cassandra:cassandra --rm winamd64/cassandra cqlsh cassandra
```
