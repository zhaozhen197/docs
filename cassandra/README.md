...
[![Build Status](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/cassandra/badge/icon) (`ppc64le/cassandra` build job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/cassandra/)
...
```console
$ docker run --name some-cassandra -d ppc64le/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d -e CASSANDRA_SEEDS="$(docker inspect --format='{{ .NetworkSettings.IPAddress }}' some-cassandra)" ppc64le/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --link some-cassandra:cassandra ppc64le/cassandra:tag
```
...
```console
$ docker run -it --link some-cassandra:cassandra --rm ppc64le/cassandra sh -c 'exec cqlsh "$CASSANDRA_PORT_9042_TCP_ADDR"'
```

... or (simplified to take advantage of the `/etc/hosts` entry Docker adds for linked containers):

```console
$ docker run -it --link some-cassandra:cassandra --rm ppc64le/cassandra cqlsh cassandra
```
