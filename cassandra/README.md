...
```console
$ docker run --name some-cassandra -d arm32v5/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d -e CASSANDRA_SEEDS="$(docker inspect --format='{{ .NetworkSettings.IPAddress }}' some-cassandra)" arm32v5/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --link some-cassandra:cassandra arm32v5/cassandra:tag
```
...
```console
$ docker run -it --link some-cassandra:cassandra --rm arm32v5/cassandra sh -c 'exec cqlsh "$CASSANDRA_PORT_9042_TCP_ADDR"'
```

... or (simplified to take advantage of the `/etc/hosts` entry Docker adds for linked containers):

```console
$ docker run -it --link some-cassandra:cassandra --rm arm32v5/cassandra cqlsh cassandra
```
